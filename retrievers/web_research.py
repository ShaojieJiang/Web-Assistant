...
DEFAULT_SEARCH_PROMPT = PromptTemplate(
    input_variables=["question"],
    template="""You are an assistant tasked with improving Google search queries. What is the best query for this text: ```{question}``` Output the query only and nothing else.""",
)  # TODO: include history; output no_query
...
class WebRetriever(BaseRetriever):
    ...
    def from_llm(
        ...
        llm: BaseLLM,
        search: Union[DuckDuckGoSearchAPIWrapper, GoogleSearchAPIWrapper],
    ...
    def _get_relevant_documents(
        ...
        *,
        ...
        new_query = result["text"].replace('"', "")
        logger.info(f"Questions for Google Search: {new_query}")

        # Get urls
        logger.info("Searching for relevant urls...")
        urls_to_look = []
        search_results = self.search_tool(new_query, self.num_search_results)
        logger.info("Searching for relevant urls...")
        ...
        if new_urls:
            ...
            self.vectorstore.add_documents(docs)  # TODO: Do we need this?
            self.url_database.extend(new_urls)

        # Search for relevant splits
        # TODO: make this async
        logger.info("Grabbing most relevant splits from urls...")
        docs = self.vectorstore.similarity_search(new_query)
